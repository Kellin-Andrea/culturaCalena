...
namespace mvc\validator {
  ...
  class editEventoValidatorClass extends validatorClass {
    ...
    public static function validateEdit($nameEvent, $description, $date, $date1, $address, $money, $category, $datePublic, $datePublic1, $place, $long, $id) {
      ...
      } else if (strtotime(date(config::getFormatTimestamp(), strtotime($date))) <= strtotime(date(config::getFormatTimestamp()))) {
        $flag = true;
        session::getInstance()->setFlash('inputdate', true);
      ...
      } else if (strtotime(date(config::getFormatTimestamp(), strtotime($date1))) <= strtotime(date(config::getFormatTimestamp()))) {
        $flag = true;
        session::getInstance()->setFlash('inputdate1', true);
      ...
      } else if (strtotime(date(config::getFormatTimestamp(), strtotime($datePublic))) <= strtotime(date(config::getFormatTimestamp()))) {
        $flag = true;
        session::getInstance()->setFlash('inputdatePublic', true);
      ...
      } else if (strtotime(date(config::getFormatTimestamp(), strtotime($datePublic1))) <= strtotime(date(config::getFormatTimestamp()))) {
        $flag = true;
        session::getInstance()->setFlash('inputdatePublic1', true);
